...
const DropDown = function (props) {
	...
	const onChangeText = function (event) {
		...
		if (value) {
			const filteredList = option.filter(item => item.value.startsWith(value.toLowerCase()));
			setShow(true);
		...
		<>
			...
			<InputGroup>
				<InputGroupAddon addonType='prepend'>
					<InputGroupText>
						<i className={show ? 'nc-icon nc-minimal-up' : 'nc-icon nc-minimal-down'} onClick={onClickIcon}/>
					</InputGroupText>
				</InputGroupAddon>
				<Input
