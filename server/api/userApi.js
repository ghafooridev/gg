...
const { LOBBY_ID_LEN, gameSizes } = require("../config")
...
router.post("/authenticate", validLogin, (req, res) => {
  ...
  User.findOne({ email: req.body.email }, (err, user) => {
    ...
    if (!user)
      return res
        .status(400)
        .json({ errors: "email password combination is incorrect!" })
  ...
  User.findById(req.query.userId, (err, doc) => {
    if (err)
      return res.status(400).json({
        error: `Error occured while finding user with user id: ${req.query.userId}`,
      })
    if (!doc)
      return res.status(400).json({ error: "User not found with that user id" })
...
router.put("/leaveLobby", (req, res) => {
  ...
  return res
    .status(200)
    .json({ success: "user successfully remove from lobby" })
})
