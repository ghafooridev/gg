...
module.exports = (data) => {
	...
	if (validator.isEmpty(setToString(data.username))) {
		error.username = validationMessage('username', Constant.VALIDATION_TYPE.REQUIRED);
	}

	if (validator.isLength(setToString(data.password), {min: 6})) {
		error.password = validationMessage('password', Constant.VALIDATION_TYPE.LENGTH);
	}

	if (validator.isEmpty(setToString(data.password))) {
		error.password = validationMessage('password', Constant.VALIDATION_TYPE.REQUIRED);
	}
